...
connection.on('error', (err) => {
  console.error('[NRS Queue] Redis connection error:', err.message);
});
...
export const nrsWorker = new Worker(
  ...
  async (job: Job<{ invoiceId: string; businessId: string }>) => {
    ...
    const { NRSSubmissionService } = await import('../services/nrs-submission.js');
    const svc = new NRSSubmissionService();
    await svc.submitInvoice(invoiceId, businessId);
